...
function LiveTrackingContent({ ownerId, tableNumber }: { ownerId: string, tableNumber: string }) {
  ...
  useEffect(() => {
    ...
    fetchStatus();
    intervalRef.current = setInterval(fetchStatus, 5000); // Poll every 5s
  ...
  return (
    <div className="success-page">
      <div className="success-card" style={{ maxWidth: 480 }}>
        {isCancelled ? (
          <div className="success-icon" style={{ background: 'rgba(244,63,94,0.1)', color: '#f43f5e' }}>✕</div>
        ) : currentIndex >= 2 ? (
          <div className="success-icon" style={{ background: 'rgba(34,197,94,0.1)', color: '#22c55e' }}>✅</div>
        ) : (
          <div className="success-icon" style={{ background: 'rgba(249,115,22,0.1)', color: '#f97316' }}>👨‍🍳</div>
        )}

        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.5rem' }}>
          {isCancelled ? 'Order Cancelled' : currentStatus === 'ready' ? 'Food is Ready!' : currentStatus === 'completed' ? 'Order Completed' : 'Preparing Your Order'}
        </h1>
        <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>
          {isCancelled ? 'Please speak with the staff.' : currentStatus === 'ready' ? 'Your food is on its way to your table.' : 'Sit back and relax while the kitchen prepares your food.'}
        </p>
        ...
        {!isCancelled && (
          <div style={{ marginBottom: '2.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.25rem', height: 8 }}>
              <div style={{ flex: 1, borderRadius: 99, background: currentIndex >= 0 ? 'var(--accent)' : 'var(--border)', transition: 'background 0.5s' }} />
              <div style={{ flex: 1, borderRadius: 99, background: currentIndex >= 1 ? '#f97316' : 'var(--border)', transition: 'background 0.5s' }} />
              <div style={{ flex: 1, borderRadius: 99, background: currentIndex >= 2 ? '#22c55e' : 'var(--border)', transition: 'background 0.5s' }} />
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', paddingBottom: '0.75rem', marginBottom: '0.75rem' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Table</div>
              <div style={{ fontSize: '1.25rem', fontWeight: 800 }}>#{tableNumber}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Est. Time</div>
              <div style={{ fontSize: '1.25rem', fontWeight: 800 }}>~{order?.estimatedTime || initialTime} min</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid var(--border)' }}>
            ...
            <span style={{ fontWeight: 800, color: 'var(--accent)' }}>₹{order?.totalAmount || initialTotal}</span>
          </div>
        </div>
